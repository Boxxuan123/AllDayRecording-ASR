    ...
    if auto_workflow and enable_v3:
        ...
        try:
            processor = build_workflow_processor(
                ...
                admission_mode="shadow" if workflow_shadow else "production",
            ...
            automatic_runner = AutomaticWorkflowRunner(
                ...
                configuration={
                    "admission_mode": (
                        "shadow" if workflow_shadow else "production"
                    ),
                    "config_path": str(workflow_config.resolve()),
                    "config_sha256": hashlib.sha256(
                        workflow_config.read_bytes()
                    ).hexdigest(),
                    "database_path": str(workflow_database.resolve()),
                    "profile": workflow_profile,
                    "diarization_model_path": (
                        str(workflow_diarization_model_path.resolve())
                        if workflow_diarization_model_path is not None
                        else None
                    ),
                    "backup_root": (
                        str(workflow_backup_root.resolve())
            ...
            for record in server.store.list_uploads(
                ...
                server.notify_upload_completed(record)
        except Exception:
            server.server_close()
            if automatic_runner is not None:
                automatic_runner.close()
