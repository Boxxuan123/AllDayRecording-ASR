...
首轮反馈显示唯一错误来自 `exploratory`，因此 run 17 先按 high/medium/exploratory 分层，再在层内按差值排序。它继承 11 条父亲确认，并只留下 1 条新的中对照候选（约 `00:33:27–00:33:30`）待听。11 条确认候选共 23.340 秒，加原种子为 29.310 秒；仍未正式跨过 30 秒门槛，而且后续还要扣除质量过滤损失。
