...
实际进度、测试数字和后续优先级见 [项目现状与路线图](project-status.md)。本文继续作为 V1 的范围与验收基线。
...
- 时间线整理：第一阶段使用确定性规则聚合；基础数据稳定后再接本地小型 LLM。
...
- 15～30 分钟完整人工真值集和正式准确率报告尚未完成，因此 V1 仍处于验证阶段。
...
7. 最后接桌面确认弹窗和经用户确认的日历/待办写入。

第一版只有在离线转写、时间戳、说话人和原音追溯稳定后，才进入实时阶段。
