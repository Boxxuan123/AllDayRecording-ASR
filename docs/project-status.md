...
> 盘点日期：2026-08-28  
> 当前阶段：V1 基线和本地操作台可用；[质量优先 V2 架构](v2-quality-first-architecture.md) 已冻结，V2-A 已完成并通过真实长录音验收。

## 1. 结论

项目已经越过“技术路线设计”，具备一条可实际运行的离线处理链路。它可以把一段 Watch 长录音变成可追溯的转写和事件时间线，也已经建立了“独立本人声纹 → 候选片段 → 人工真值 → 留出/负样本库”的闭环。

V1 证明了工程闭环，但其“VAD 段即转写段和单一说话人”的数据模型无法可靠表达片段内换人、重叠讲话和多模型假设。V2-A 已完成不可变 source/session、run 输入快照、完整性审计和跨源逻辑窗口；下一阶段建立与 VAD segment 解耦的连续时间真值和多 run 对比，再接入 Qwen3-ASR-1.7B、强制对齐、第二 ASR 假设和新的说话人时间轴。
...
| 数据库迁移 | V2-A 已完成 | schema v4；升级前自动备份，V1 数据回填且历史计数不变 |
...
6. schema v4 已建立 source/session 和 run 输入基础；V2-B 及后续结构变化必须新增迁移版本，不能修改既有 migration。
...
- 28 项单元测试全部通过，其中包含 schema v3→v4 自动备份回填、不可变字段保护、源文件篡改检测、跨源窗口和本地网页回归测试。
...
下一轮为 **V2-B：连续时间真值与基准框架**。重点是把真值从当前 `segment_id` 解耦，锚定不可变源哈希和连续时间范围，并补齐 VAD Miss/FA、DER/JER、对齐误差和关键实体指标。
