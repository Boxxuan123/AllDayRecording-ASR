...
> 盘点日期：2026-08-29
> 当前阶段：V1 基线和本地操作台可用；V2-D Community-1 与身份参考层已完成；V2-E.0 本地语义证据契约和真实 run 18 已完成。V2-C.2 前 5 块是开发真值且 `speaker` 完整度为 `none`，不能冒充 V2-D DER/JER 真值；真实云端 LLM 尚未接入。
...
V2-E.0 已把 committed token、说话人归属、不确定性和原音坐标冻结成供应商无关的语义交换。当前只运行确定性本地 mock，明确不产生事实/行动、不联网、不上传音频；它验证的是安全接口、证据链和审核工作流，而不是 LLM 摘要质量。
...
| 语义证据 | V2-E.0 已完成 | 不联网 provider、不可变请求/响应/基础候选、追加式人工审核、逐事件原音回听；真实云端 provider 待接入 |
...
| 本地网页 | V2-E.0 已扩展 | 逐段试听标注、说话人时间轴、语义证据审核、评测、候选审核、一键运行和历史；只监听回环地址 |
...
| V2-E.0 run 18 | V2-C run 10 + V2-D run 11；2,192 token、35 个证据事件、36 个候选 |
| V2-E.0 隐私边界 | `local_mock`；网络/音频字节/源路径均为否；facts/actions=0/0 |
...
4. V2-E.0 只完成语义契约和证据审核，不等于日级摘要已经可用；真实云端模型、最小化上传字段、数据保留策略、幻觉/遗漏评测仍需在 V2-E.1 明确后才能调用。
...
- 全量 52 个单元测试通过，覆盖 schema 自动备份、冻结 ASR/token/source/diarization/attribution/semantic 防篡改、跨源引用、V2-C.1/V2-C.2 盲标约束、非连续 review-region、Oracle 快照、双 CER、paired bootstrap、VAD/DER/JER、V2-C.3 门控、V2-D 同时说话与身份审核，以及 V2-E.0 隐私契约、事件组织、provider 证据防替换和追加修订。
...
- schema v9→v10 升级前已自动备份真实 SQLite；run 18 将 ASR run 10、diarization run 11、2,192 个 token 和原音证据组织为 35 个事件与 36 个不可变候选。请求不含 M4A 名称、路径或音频字节，原文件仍为 79,826,205 字节且 SHA-256 不变。
...
1. [x] V2-E.0 定义供应商无关、带证据的本地语义交换，不让 LLM 覆盖原始 ASR。
...
V2-E.0 已完成 schema v10、本地 mock provider、不可变交换/候选、追加式人工修订、CLI/API 和网页证据审核。下一步不是把 mock 文案当成日记，而是先确定真实云端供应商与隐私策略，再以同一证据契约实现 V2-E.1，并建立人工审核集衡量事实一致性和幻觉。
