...
python tools/export_sync_phase2a_review.py --phone <repo> --output outputs/sync-phase2a-closeout-review.zip
...
def selected(label, path):
    if label == 'phone':
        return (path.endswith('.ets') and path.startswith((
            ...
            or path in ('tests/ui/main.py', 'tests/ui/run_phase2a.py', 'tests/ui/run_phase1.py',
    ...
    return (path.startswith('src/allday_asr/v3/') and path.endswith('.py')
            or path.startswith('tests/') and path.endswith('.py') and any(key in path for key in (
                'transfer', 'device', 'review_audio', 'phone_voice', 'sync_phase2a', 'v34_open_speaker', '__init__'))
            or path in ('tools/sync_phase1_test_receiver.py', 'tools/sync_phase2a_device_receiver.py',
                        'tools/export_sync_phase2a_review.py', 'docs/sync-phase2a-acceptance.md',
...
def main():
    ...
    for label, root, baseline in (
        ('phone', args.phone, '763f95a01bac7b0ff5e8ba5bc45bd0b8340e8a99'),
        ('desktop', desktop, '895c294f32480b7b34817e9dbab86b1a86ddf443'),
    ):
    ...
    manifest = {'repositories': versions, 'files': {name: digest(data) for name, data in sorted(entries.items())},
                'scope': 'Committed selected source/tests/docs and full phase2A text diffs. No runtime data or HAP.',
                'reproduce': 'python tools/export_sync_phase2a_review.py --phone <phone-repo> --output outputs/sync-phase2a-closeout-review.zip'}
