...
test('keeps the review inbox meaningful, filterable, and directly actionable', () => {
  ...
  assert.equal(voiceReviewActions.includes("!complete"), true)
  assert.equal(voiceReviewActions.includes('VoiceSampleAudition'), true)
  assert.equal(voiceReviewActions.includes('markHeard(candidate.prototype_id)'), false)
