...
V3.0-G 已把 V3 Core、版本化 Desktop API 和独立前端设为默认入口。V3.1-A/B/C 已完成可靠统一对话时间线；V3.2 已完成证据、事件、记忆三层数据架构；V3.3 完成智能提醒与 Codex 文本提取闭环；V3.4 进一步接通本机 CAM++ 说话人 embedding、录音内与跨录音匿名聚类、代表片段回听、已有/新人物关联、合并、拆分、媒体声忽略、撤销和事件人物引用重绑。未知始终是合法结果，自动识别只形成候选或“可能”提示，只有人工确认才会把多个不可变原型写入稳定声纹库；匿名簇不进入 Phone 投影。Codex 只接收所选 utterance 和同会话活动提醒，不上传音频或本地路径，在专用空目录中以只读沙箱、拒绝审批方式运行；候选默认仍须审核。V2-A.1 至 V2-E.0.2 的模型、证据和评测能力仍完整保留，但只通过 `allday-asr legacy ...` 作为可执行回退或 V3 adapter 使用；Legacy 网页强制 query-only，拒绝所有写请求。现有本人校准只有 2 个正例，且真实连续真值均为 `alignment=none`，所以旧数据身份保持 `unknown`、真实跨切片发布门禁保持拒绝；系统不会把候选分数或缺失真值冒充成功证据。
...
当前完整 V3.4 的契约、Core schema、Phone projection schema 和默认入口冻结在
